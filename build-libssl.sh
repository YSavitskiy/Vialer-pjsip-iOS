...
{
  echo "  Configure..."
  local ssl_optsNew="no-ssl2 no-ssl3 no-weak-ssl-ciphers no-shared no-dso no-hw no-engine no-srp no-idea"

  set +e
  if [ "${LOG_VERBOSE}" == "verbose" ]; then
    ./Configure ${LOCAL_CONFIG_OPTIONS} ${ssl_optsNew} no-tests | tee "${LOG}"
  else
    (./Configure ${LOCAL_CONFIG_OPTIONS} ${ssl_optsNew}  no-tests > "${LOG}" 2>&1) & spinner
  fi
